...
| `hitMargin`      | `number`                   | `24`      | Extra hit-area in px on every side of the wrapper. Lets you overshoot the visible edge and still pin the back fully revealed. Set `0` to disable. |
| `triggerParent`  | `boolean`                  | `false`   | Listen on the wrapper's **parent element** and normalize the cursor X to the parent's bounds. The whole surrounding card becomes the hit area, scrubbing `0` → `1` across its width. |
| `scroll`         | `boolean`                  | `false`   | Drive offset from the element's scroll progress through the viewport instead of the cursor. As the wrapper passes from viewport-bottom to top, offset goes `0` → `1`. |
